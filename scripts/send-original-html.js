...
(async () => {
  try {
    ...
    const subject = 'Original template HTML - Vercam';
    const message = 'Ciao, questo è il corpo testuale di fallback del template originale.';
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                      <tr>
                        ...
                        <td align="right" style="font-size:13px;color:#6b7880;">
                          <p style="margin:0"><strong>Vercam</strong><br><a href="mailto:lucalupi03@gmail.com" style="color:#1b6f57;text-decoration:none;">info@asdborgovercelli.it</a></p>
                        </td>
                      </tr>
                    </table>
                  </td>
    ...
    console.log('Invio template originale a:', to);
    const info = await sendMail({ to, subject, text: message, html });
    ...
    console.error('Errore invio original HTML:', err);
    if (transporter && typeof transporter.close === 'function') transporter.close();
    process.exit(1);
  }
})();
